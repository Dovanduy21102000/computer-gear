        ...
        @foreach ($orderTabs as $key => $label)
            <li class="nav-item" role="presentation">
                <a class="nav-link @if ($loop->first) active @endif" id="{{ $key }}-tab"
                    data-toggle="tab" href="#{{ $key }}" role="tab">{{ $label }}</a>
            </li>
        @endforeach
    </ul>
    <!-- Ô tìm kiếm -->
                                ...
                                <div class="card-body">
                                    <div class="order-header">
                                        <div class="order-info">
                                            <span class="font-weight-bold">Ngày mua:
                                            </span>{{ $order->created_at->format('d/m/Y') }}
                                            <span class="ml-3 font-weight-bold">Mã đơn hàng: </span><span
                                                class="text-primary">{{ $order->code }}</span>
                                            <span class="ml-3 text-muted">{{ $order->shipping_user_name }} |
                                    ...
                                    <div class="order-products mb-3">
                                        @foreach ($order->items ?? [] as $item)
                                            <div class="order-product-item">
                                                <img src="{{ $item->product->thumbnail ? asset('storage/' . $item->product->thumbnail) : 'https://via.placeholder.com/56' }}"
                                                    alt="{{ $item->product->name }}" class="order-product-img">
                                                <div class="flex-grow-1">
                                                    <div class="order-product-name">{{ $item->product->name }}</div>
                                                    <div class="order-product-qty">Số lượng: {{ $item->quantity }}
                                        ...
                                        @endforeach
                                        @if ($order->status === 'completed')
                                            <form action="{{ route('client.orders.confirmReceived', $order->code) }}"
                                                method="POST" class="mt-3">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-success">✅ Tôi đã nhận được
                                                    hàng</button>
                                            </form>

                                            <div id="refuse-section" class="mt-3">
                                                <button class="btn btn-outline-danger" onclick="toggleRefuseForm()">❌
                                                    Không nhận
                                                    hàng</button>

                                                <form id="refuse-form"
                                                    action="{{ route('client.orders.cancel', $order->code) }}"
                                                    method="POST" class="mt-3 d-none">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="form-group">
                                                        <label for="refuse_reason"><strong>Lý do không
                                                                nhận:</strong></label>
...
<style>
    ...
    @media (max-width: 600px) {
        ...
        .order-card .d-flex .mb-2 {
            margin-bottom: 0.5rem !important;
        }

        .order-card .d-flex .ml-3 {
